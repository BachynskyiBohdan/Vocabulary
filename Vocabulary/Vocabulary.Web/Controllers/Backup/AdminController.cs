...
namespace Vocabulary.Web.Controllers
{
    ...
    public class AdminController : AsyncController
    {
        ...
        public AdminController(IUsersExampleRepository usersExampleRepository,
        ...
        public ActionResult ExampleMainPage(decimal phraseId, int count = 20, int page = 1)
        {
            var phrase = _globalPhraseRepository.GlobalPhrases.First(p => p.Id == phraseId);
            Session["GlobalPhrase"] = phrase.Phrase;
            Session["GlobalPhraseId"] = phrase.Id;
        ...
        public async Task<ActionResult> EditExample(GlobalExample example, HttpPostedFileBase audio)
        {
            if (ModelState.IsValid)
            {
                return Task.Factory.StartNew(() =>
                {
                    ...
                    TempData["Error"] = string.Format("Example with id = {0} was succesfully updated", example.Id);
        ...
        [HttpPost]
        public ActionResult AddGlossary(GlossaryViewModel glossary)
        {
            if (ModelState.IsValid)
            {
        ...
        [HttpPost]
        public ActionResult EditGlossary(Glossary glossary)
        {
            if (ModelState.IsValid)
            {
        ...
        private async Task<FullPhraseViewModel> GetFullPhrase(HtmlNode document)
        {
            ...
            fp.Phrase.Frequency =
                int.Parse(document.QuerySelector(".trans_sound #rank_box a").InnerText.Replace(" ", ""));
